...
public class AprioriMyself {
    ...
    public static  double MIN_SUPPROT = 0.02;//最小支持度百分比
    public static   double MIN_CONFIDENCE=0.6;//最小置信度
    private static boolean endTag = false;//循环状态，迭代标识
    public static List<List<String>> record = new ArrayList<List<String>>();//数据集
    public static  List<List<String>> frequentItemset=new ArrayList<>();//存储所有的频繁项集
    public static List<Mymap> map = new ArrayList();//存放频繁项集和对应的支持度技术
    ...
    public static void AssociationRulesMining( List<BookRecommand> bookRecommands )//关联规则挖掘
    {
        for(int i=0;i<frequentItemset.size();i++)
        {
            ...
            if(tem.size()>1) {
                ...
                for (int j = 0; j < AllSubset.size(); j++) {
                    ...
                    double conf = isAssociationRules(s1, s2, tem,bookRecommands);
                    if (conf > 0)
    ...
    public  static  double isAssociationRules( List<String> s1, List<String> s2, List<String> tem, List<BookRecommand> bookRecommands )//判断是否为关联规则
    {
        double confidence=0;
        int counts1;
        int countTem;
